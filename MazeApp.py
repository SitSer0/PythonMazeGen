...
class MazeApp(tk.Tk):
    ...
    """
    Класс MazeApp представляет графический интерфейс для игры в лабиринт.

    Attributes:
    - maze (Maze): Экземпляр класса Maze, представляющий текущий лабиринт.
    - cell_size (int): Размер ячейки лабиринта в пикселях.
    - start_x, start_y (int): Координаты стартовой точки игрока.
    - finish_x, finish_y (int): Координаты конечной точки лабиринта.
    - path (list): Путь для решения лабиринта.
    - solver (Solver): Экземпляр класса Solver для решения лабиринта.
    - file (File): Экземпляр класса File для сохранения и загрузки состояния лабиринта.
    - type_game (str): Тип игры, "solo" или "multi".

    Methods:
    - draw_maze(): Отрисовывает текущий лабиринт на холсте.
    - draw_player(player, color=None): Отрисовывает игрока на холсте.
    - draw_green_cell(x, y, cell_size): Отрисовывает зеленую ячейку на холсте.
    - draw_red_cell(x, y, cell_size): Отрисовывает красную ячейку на холсте.
    - draw_yellow_line_up(x, y, cell_size): Отрисовывает желтую вертикальную линию на холсте.
    - draw_yellow_line_right(x, y, cell_size): Отрисовывает желтую горизонтальную линию на холсте.
    - draw_yellow_line_down(x, y, cell_size): Отрисовывает желтую вертикальную линию внизу на холсте.
    - draw_yellow_line_left(x, y, cell_size): Отрисовывает желтую горизонтальную линию влево на холсте.
    - move_up(event): Обрабатывает движение игрока вверх.
    - move_down(event): Обрабатывает движение игрока вниз.
    - move_left(event): Обрабатывает движение игрока влево.
    - move_right(event): Обрабатывает движение игрока вправо.
    - move_up_player2(event): Обрабатывает движение второго игрока вверх (для мультиплеера).
    - move_down_player2(event): Обрабатывает движение второго игрока вниз (для мультиплеера).
    - move_left_player2(event): Обрабатывает движение второго игрока влево (для мультиплеера).
    - move_right_player2(event): Обрабатывает движение второго игрока вправо (для мультиплеера).
    - redraw_player(): Перерисовывает положение игрока на холсте.
    - game_over(message): Выводит сообщение о завершении игры.
    - check_finish(): Проверяет, достиг ли игрок конечной точки лабиринта.
    - restart_game_dfs(): Перезапускает игру с лабиринтом, сгенерированным с использованием DFS.
    - restart_game_prim(): Перезапускает игру с лабиринтом, сгенерированным с использованием алгоритма Прима.
    - restart_game_kruskal(): Перезапускает игру с лабиринтом, сгенерированным с использованием алгоритма Краскала.
    - draw_path(): Отрисовывает путь на холсте.
    - solve_mazee(): Решает лабиринт и отрисовывает путь.
    - saving(): Сохраняет текущее состояние лабиринта.
    - loading(): Загружает ранее сохраненное состояние лабиринта.
    - ready_to_restart(): Подготавливает игру к перезапуску с новым лабиринтом и стартовыми точками.
    """
